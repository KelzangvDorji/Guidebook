...
const { PDF_BUCKET } = require('../middleware/upload');
...
async function getOwnedBookOr403(req, res) {
  const bookId = Number(req.params.bookId);
  if (!Number.isInteger(bookId)) {
    res.status(400);
    return null;
  }
  const book = await db.get(`SELECT * FROM books WHERE id = ? AND status = 'active'`, [bookId]);
  if (!book) {
    res.status(404);
    return null;
  }
...
router.get('/read/:bookId', requireAuth, async (req, res) => {
  const found = await getOwnedBookOr403(req, res);
  if (!found) {
...
router.post('/api/preview/:bookId/token', previewLimiter, async (req, res) => {
  const bookId = Number(req.params.bookId);
  const book = await db.get(`SELECT id, preview_pages FROM books WHERE id = ? AND status = 'active'`, [bookId]);
  if (!book || book.preview_pages < 1) return res.status(404).json({ error: 'No preview available' });
  const token = await issuePreviewToken(book.id);
  res.json({ token });
});
...
router.get('/api/preview/:bookId/stream', previewLimiter, async (req, res) => {
  ...
  if (!Number.isInteger(bookId) || !(await consumePreviewToken(token, bookId))) {
    return res.status(403).json({ error: 'Invalid or expired preview token' });
  }
  const book = await db.get(`SELECT pdf_filename, preview_pages FROM books WHERE id = ?`, [bookId]);
  if (!book) return res.status(404).json({ error: 'Not found' });

  try {
    const pdfBuffer = await storage.getObjectBuffer(PDF_BUCKET, book.pdf_filename);
    const preview = await buildPreviewBuffer(pdfBuffer, book.preview_pages);
    res.set({
...
router.post('/api/reader/:bookId/token', requireAuth, async (req, res) => {
  const found = await getOwnedBookOr403(req, res);
  if (!found) return res.status(res.statusCode).json({ error: 'Not authorized for this book' });
  const token = await issueReaderToken(req.user.id, found.book.id, found.purchase.id);
...
router.get('/api/reader/:bookId/stream', requireAuth, async (req, res) => {
  const bookId = Number(req.params.bookId);
  const token = String(req.query.token || '');
  if (!Number.isInteger(bookId) || !(await consumeReaderToken(token, req.user.id, bookId))) {
    return res.status(403).json({ error: 'Invalid or expired reader token' });
  }
  const book = await db.get(`SELECT pdf_filename FROM books WHERE id = ?`, [bookId]);
  if (!book) return res.status(404).json({ error: 'Not found' });

  let stream;
  try {
    stream = await storage.getObjectStream(PDF_BUCKET, book.pdf_filename);
  } catch {
